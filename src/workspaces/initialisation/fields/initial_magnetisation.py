...
"""
Project: UbermagGUI
Path:    include/Uberwidgets/region_designer_interface/initialisation/fields/initial_magnetisation.py
...
class DefineSystemInitialMagnetisation:
    ...
    def build(self, controls_panel) -> widgets.VBox:
        """
        Build the UI:
          - choose from available domain (today just “main”)
          - enter (mₓ, mᵧ, m𝓏)
          - enter saturation Mₛ
        ...
        html_explainer = widgets.HTML(
            "<b>Set mesh's initial fields state.</b>"
        )
        ...
        html_sat_mag = widgets.HTML(
            value="which is scaled by the saturation fields ",
        )
