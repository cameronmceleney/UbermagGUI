...
class ViewportArea:
    ...
    def build(self):
        ...
        #grid._grid_template_columns = "auto"

        # Place in build(), not __init__, to further protect against widget update failures
        self._init_fig_widget()

        # wrap fig in a Box so grid_area is applied correctly
        fig_container = widgets.HBox(
            [self.fig],
            layout=widgets.Layout(
                #display='flex',
                #flex="0 1 auto",
        ...
        toolbar = widgets.HBox(
            [widgets.Button(description="Reset Camera", layout=widgets.Layout(width="auto"))],
            layout=widgets.Layout(
                #display='flex',
                #flex='0 1 auto',
                #width='auto', height='auto',
