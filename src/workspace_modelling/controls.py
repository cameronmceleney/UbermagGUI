...
ControlsPanel:
 - Outliner
 - Modelling tab (vertical ToggleButtons of feature_modelling)
 - Properties & Options
"""
...
class ControlsPanel:
    def __init__(
        ...
        # 2) instantiate each modelling panel
        self.panels_model = {
            'Domain': CreateDomain(),
        ...
        self.panels_system_init = {
            'Mesh': DomainMesh(),
            'Initial magnetisation': DefineSystemInitialMagnetisation(),
    ...
    def register_system_callbacks(self):
        ...
        # Initial‑magnetisation panel → set initial magnetisation field
        init_mag_panel = self.panels_system_init.get('Initial magnetisation')
        if init_mag_panel:
    ...
    def update_outliner(self):
        ...
        # If an initial‐magnetisation field exists, show its value & norm:
        if self.init_mag:
    ...
    def _on_init_mag_created(self, field):
        ...
        # update outliner (or other UI) as desired
        self.update_outliner()
    ...
    def register_mesh_listener(self, cb):
        """ Builder (or anyone) can listen for new meshes: cb(mesh). """
        self._mesh_listeners.append(cb)

    def register_init_mag_listener(self, cb):
        """
        Register an external listener to be called when initial magnetisation is defined.
        """
